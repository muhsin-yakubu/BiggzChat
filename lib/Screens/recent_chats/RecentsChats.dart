...
class RecentChatsState extends State<RecentChats> {
    ...
    if (user[Dbkeys.phone] == currentUserNo) {
      ...
      return StreamBuilder(
        ...
        builder: (context, AsyncSnapshot<MessageData> unreadData) {
          int unread = unreadData.hasData && unreadData.data!.snapshot.docs.isNotEmpty ? unreadData.data!.snapshot.docs.where((t) => t[Dbkeys.timestamp] > unreadData.data!.lastSeen).length : 0;
          return Theme(
                  ...
                  ListTile(
                      ...
                      onTap: () {
                        if (_cachedModel!.currentUser![Dbkeys.locked] != null && _cachedModel!.currentUser![Dbkeys.locked].contains(user[Dbkeys.phone])) {
                          ...
                          ChatController.authenticate(_cachedModel!, getTranslated(context, 'auth_neededchat'),
                              state: state, shouldPop: false, type: Fiberchat.getAuthenticationType(biometricEnabled, _cachedModel), prefs: widget.prefs, onSuccess: () {
                            state.pushReplacement(new MaterialPageRoute(
                                builder: (context) => new ChatScreen(prefs: widget.prefs, unread: unread, model: _cachedModel, currentUserNo: currentUserNo, peerNo: user[Dbkeys.phone] as String?)));
                          });
                        } else {
                          Navigator.push(
                              context,
                      ...
                      trailing: unread != 0
                          ? Container(
                              ...
                              decoration: new BoxDecoration(
                                ...
                                color: user[Dbkeys.lastSeen] == true ? Colors.green[400] : Colors.blue[400],
                              ),
                            )
                          : Container(
                              ...
                              decoration: new BoxDecoration(
                                shape: BoxShape.circle,
                                color: user[Dbkeys.lastSeen] == true ? Colors.green[400] : Colors.redAccent,
                              ),
                            )),
                  Divider(),
                ],
  ...
  Stream<MessageData> getUnread(Map<String, dynamic> user) {
    ...
    unreadSubscriptions.add(FirebaseFirestore.instance.collection(DbPaths.collectionmessages).doc(chatId).snapshots().listen((doc) {
      if (doc[currentUserNo!] != null && doc[currentUserNo!] is int) {
        unreadSubscriptions.add(FirebaseFirestore.instance.collection(DbPaths.collectionmessages).doc(chatId).collection(chatId).snapshots().listen((snapshot) {
  ...
  _chats(Map<String?, Map<String, dynamic>?> _userData, Map<String, dynamic>? currentUser) {
    return Consumer<List<GroupModel>>(
        builder: (context, groupList, _child) => Consumer<List<BroadcastModel>>(builder: (context, broadcastList, _child) {
                ...
                children: [
                  Container(
                      child: _streamDocSnap.isNotEmpty
                              ...
                              builder: (context, snapshot) {
                                if (_filter.text.isNotEmpty || snapshot.hasData) {
                                  ...
                                  if (filtered.isNotEmpty)
                                    ...
                                    return ListView(physics: AlwaysScrollableScrollPhysics(), shrinkWrap: true, children: [
                                      Padding(
                                    ...
                                    if (_streamDocSnap[index].containsKey(Dbkeys.groupISTYPINGUSERID)) {
                                              ...
                                              ListTile(
                                                  ...
                                                  Navigator.push(
                                                      ...
                                                      new MaterialPageRoute(
                                                          builder: (context) => new GroupChatPage(
                                                              model: _cachedModel!,
                                                              prefs: widget.prefs,
                                                              joinedTime: _streamDocSnap[index]['${widget.currentUserNo}-joinedOn'],
                                                              currentUserno: widget.currentUserNo!,
                                                              groupID: _streamDocSnap[index][Dbkeys.groupID])));
                                                },
                                                trailing: StreamBuilder(
                                                  stream: FirebaseFirestore.instance
                                                      ...
                                                      .where(Dbkeys.groupmsgTIME, isGreaterThan: _streamDocSnap[index][widget.currentUserNo])
                                                      .snapshots(),
                                                  builder: (BuildContext context, AsyncSnapshot<QuerySnapshot<dynamic>> snapshot) {
                                                    if (snapshot.connectionState == ConnectionState.waiting) {
                                                      ...
                                                      return Container(
                                                        child: Text('${snapshot.data!.docs.length}', style: TextStyle(fontSize: 14, color: Colors.white, fontWeight: FontWeight.bold)),
                                                        padding: const EdgeInsets.all(7.0),
                                                        decoration: new BoxDecoration(
                                                          shape: BoxShape.circle,
                                      ...
                                      return Theme(
                                          ...
                                          child: Column(
                                            children: [
                                              ListTile(
                                                leading: customCircleAvatarBroadcast(url: _streamDocSnap[index][Dbkeys.broadcastPHOTOURL], radius: 22),
                                                title: Text(
                                                  _streamDocSnap[index][Dbkeys.broadcastNAME],
                                                  style: TextStyle(
                                                    color: fiberchatBlack,
                                                    fontSize: 16,
                                                  ),
                          ...
                          : ListView(physics: NeverScrollableScrollPhysics(), shrinkWrap: true, padding: EdgeInsets.all(0), children: [
                              Padding(
                                  padding: EdgeInsets.only(top: MediaQuery.of(context).size.height / 3.5),
                                  child: Center(
                                    child: Padding(
                                        padding: EdgeInsets.all(30.0),
                                        child: Text(groupList.length != 0 ? '' : getTranslated(context, 'startchat'),
                                            textAlign: TextAlign.center,
                                            style: TextStyle(
                                              fontSize: 16,
                                              height: 1.59,
      ...
      child: ScopedModelDescendant<DataModel>(builder: (context, child, _model) {
        ...
        return Scaffold(
          ...
          floatingActionButton:
              ...
              //     ?
              Padding(
            padding: EdgeInsets.only(bottom: IsBannerAdShow == true && observer.isadmobshow == true ? 60 : 0),
